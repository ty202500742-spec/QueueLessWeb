...
var myWindow = currentUser.window || "Window 1 - Releasing of Payments / TES";
document.getElementById("currentUserName").textContent = currentUser.name || "Cashier Staff";

// Cashier windows list
var cashierWindows = [
...
function checkPositionNotifications() {
    ...
    var myQueue = all.filter(function(q) {
       return q.window === myWindow && q.department === "cashier";
    });
...
function renderDashboard() {
    ...
    var all = getQueue();
    var myQueue = all.filter(function(q) { return q.window === myWindow && q.department === "cashier"; });
    ...
    if (currentTicket) {
        ...
        html += '<button class="btn-done" onclick="markDone()">Confirm &amp; Done</button>';
